...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['ship_misc_misc'],  # Valid: ['pedestrian']
            'CLASSES_NAME_TO_CLASS_ID': None, #class_name_to_class_id
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = self.config['CLASSES_TO_EVAL']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only pedestrian class is valid.')

        with open(self.config['CLASSES_NAME_TO_CLASS_ID'][0], 'r') as file:
            classmap = json.load(file)
        self.class_name_to_class_id = classmap
        # Class map
        # {'ship_misc_misc': 0, 'fishing_ship': 1, 'buoy': 2, 'cargo_ship': 3, 'wind_farm': 4, 'other_structure': 5,
        #  'ferry': 6, 'boat': 7, 'lighthouse': 8, 'yacht': 9, 'bulk_carrier': 10, 'floating_drilling_facility': 11,
        #  'other_float': 12, 'leisure_ship_misc': 13, 'scout_boat': 14, 'tanker': 15, 'jet_ski': 16, 'light_pole': 17,
        #  'stationary_drilling_facility': 18, 'light_beacon': 19, 'beacon': 20, 'warship_misc': 21}
